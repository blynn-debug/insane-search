...
> **운영 중인 시스템이다.** 손대기 전에 아래 세 가지를 먼저 확인할 것.
>
> | | |
> |---|---|
> | 점검하고 싶다 | `bash health_check.sh` (비파괴, 읽기만) |
> | 지금 상태가 궁금하다 | `journalctl -u valley-health -n 30 --no-pager` |
> | 절대 하지 말 것 | `tests/ec2_suite.sh` 를 주기 실행에 거는 것 (파괴적 테스트) |
>
> 사람이 개입할 일은 **Google 세션이 만료될 때 1회 재로그인**뿐이고,
> 30일 전부터 메일로 경고가 온다. 그 외에는 전부 자동이다.
